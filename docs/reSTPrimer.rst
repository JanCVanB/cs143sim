...
The paragraph (ref <paragraphs>) is the most basic block in a reST
...
Literal code blocks (ref <literal-blocks>) are introduced by ending a
...
Two forms of tables are supported.  For *grid tables* (ref
...
*Simple tables* (ref <simple-tables>) are easier to write, but
...
* Admonitions: attention, caution, danger,
  ...
  admonition <admonitions>.  (Most themes style only "note" and
  "warning" specially.)

* Images:

  - image (see also Images_ below)
  - figure (an image with caption and optional legend)

* Additional body elements:

  - contents <table-of-contents> (a local, i.e. for the current file
...
reST supports an image directive (ref <image>), used like so::
...
For footnotes (ref <footnotes>), use ``[#name]_`` to mark the footnote
...
Standard reST citations (ref <citations>) are supported, with the
...
reST supports "substitutions" (ref <substitution-definitions>), which
...
See the reST reference for substitutions <substitution-definitions>
for details.
...
* **Separation of inline markup:** As said above, inline markup spans must be
  ...
  the reference <substitution-definitions> for the details.
