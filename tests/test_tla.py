...
def test_tla_tcp_tahoe():
    ...
    F1=Flow(env=env, source=H1, destination=H2, amount=1024*PACKET_SIZE-10)
    F1.tla=TCPTahoe(env=env, flow=F1)
    
    H1.flows.append(F1)
    H2.flows.append(F1)
    
    L1=Link(env=env, source=H1, destination=H2, delay=10, rate=0, buffer_capacity=5*PACKET_SIZE)
    L2=Link(env=env, source=H2, destination=H1, delay=10, rate=0, buffer_capacity=64*PACKET_SIZE)
